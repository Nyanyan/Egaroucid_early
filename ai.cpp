        ...
        inline int mcts(const int *board, int former_idx, int player){
            ...
            for (const int& cell : board_c.vacant_lst){
                if (search_c::nodes[board_idx].children[cell] != -1){
                    cerr << board_c.coord_str(cell) << " " << search_c::nodes[board_idx].p[cell] << " " << -search_c::nodes[search_c::nodes[board_idx].children[cell]].w / search_c::nodes[search_c::nodes[board_idx].children[cell]].n << " " << search_c::nodes[search_c::nodes[board_idx].children[cell]].n << endl;
...
int main(){
    ...
    search_c.first_search(first_tl);
    if (board_c.ai_player == 0){
        ...
        policy = book_c.get_val_hash(first_board);
        cerr << "FIRST direction " << board_c.direction << endl;
        cerr << "book policy " << policy << endl;
        cout << board_c.coord_str(policy) << endl;
    }
    ...
    while (true){
        ...
        */
        if (board_c.n_stones < book_stones){
            ...
            cerr << "book policy " << policy << endl;
            if (policy != -1){
                board_c.move(board, n_board, policy);
                swap(board, n_board);
                ++board_c.n_stones;
                mcts_idx = search_c.book_mcts(board, mcts_idx, 1 - board_c.ai_player);
                cout << board_c.coord_str(policy) << endl;
                continue;
            }
        }
