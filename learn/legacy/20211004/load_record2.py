...
def collect_data(num, s):
    ...
    if rv.player != 0:
        result = -result
    return grid_str + ' ' + str(result) + ' ' + str(policy)
...
shuffle(games)
with open('third_party/records_shuffled.txt', 'w') as f:
    for line in games:
        f.write(line + '\n')
'''
with open('third_party/records_shuffled.txt', 'r') as f:
    raw_data = f.read()
games = [i for i in raw_data.splitlines()]
print(len(games))
already_unzip = 0
...
if input('sure?:') != 'yes':
    exit()
with open('learn_data2/data.txt', 'a') as f:
    for i in trange(already_unzip, len(games)):
        f.write(collect_data(1, games[i]) + '\n')