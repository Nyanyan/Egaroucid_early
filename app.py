...
app = Flask(__name__)
limiter = Limiter(app, key_func=get_remote_address, default_limits=["5000 per day", "500 per hour"])
...
def call_ai():
    ...
    try:
        ...
        if ai_player != 0 and ai_player != 1:
            return jsonify(values=json.dumps({"r": -3, "c": -1, "s": 0.0}))
    except:
        return jsonify(values=json.dumps({"r": -4, "c": -1, "s": 0.0}))
    try:
        tl = int(req["tl"])
        if tl < 2 or 200 < tl:
            return jsonify(values=json.dumps({"r": -5, "c": -1, "s": 0.0}))
    except:
        return jsonify(values=json.dumps({"r": -6, "c": -1, "s": 0.0}))
    stdin = str(ai_player) + '\n' + str(tl) + '\n'
    for y in range(hw):
        for x in range(hw):
